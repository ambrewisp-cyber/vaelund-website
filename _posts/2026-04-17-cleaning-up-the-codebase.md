...
### Fixed asset loading in release

Assets worked in debug but not in release. The paths were wrong for how the release build is invoked. Fixed that.

### Auth server on 0.0.0.0

It was only listening on localhost so external clients could not reach it. Now it binds to all interfaces.
